...
class Sher31 extends StatelessWidget {
  Future<String> _matnFayldanOqish() async {
    String matn = await rootBundle.loadString('assets/31.txt');
    return matn;
  }
  const Sher31({Key? key}) : super(key: key);
  @override
                      ...
                      builder:
                        ...
                        if (snapshot.hasData) {
                          return Center(child: FittedBox(
                              fit: BoxFit.fill,
                              child: Text(snapshot.data.toString(),style: TextStyle(fontSize: 20,color: Colors.white,fontWeight: FontWeight.bold),)));
                        } else {
