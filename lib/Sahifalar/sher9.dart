...
class Sher9 extends StatelessWidget {
  Future<String> _matnFayldanOqish() async {
    String matn = await rootBundle.loadString('assets/9.txt');
    return matn;
  }
  const Sher9({Key? key}) : super(key: key);
  @override
                      ...
                      builder:
                        ...
                        if (snapshot.hasData) {
                          return Center(child: FittedBox(
                              fit: BoxFit.fill,
                              child: Text(snapshot.data.toString(),style: TextStyle(fontSize: 18,fontWeight: FontWeight.bold,color: Colors.white),)));
                        } else {
