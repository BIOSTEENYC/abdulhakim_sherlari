...
class Sher25 extends StatelessWidget {
  Future<String> _matnFayldanOqish() async {
    String matn = await rootBundle.loadString('assets/25.txt');
    return matn;
  }
  const Sher25({Key? key}) : super(key: key);
  @override
                      ...
                      builder:
                        ...
                        if (snapshot.hasData) {
                          return Center(child: FittedBox(
                              fit: BoxFit.fill,
                              child: Text(snapshot.data.toString(),style: TextStyle(fontSize: 20,fontWeight: FontWeight.bold),)));
                        } else {
